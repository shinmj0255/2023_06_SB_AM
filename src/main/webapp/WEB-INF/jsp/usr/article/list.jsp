		...
		<div class="container mx-auto">
			<div class="mb-2 flex justify-between">
				...
				<div>
					<form>
						...
						<select data-value="${searchKeywordType }" class="select select-warning select-sm w-28" name="searchKeywordType">
							<option value="title">제목</option>
							<option value="body">내용</option>
							<option value="title,body">제목 + 내용</option>
						</select>
						<input class="ml-2 input input-bordered input-warning input-sm w-64" type="text" name="searchKeyword" placeholder="검색어를 입력해주세요" maxlength="20" value="${searchKeyword }"/>
						<button class="ml-2 btn btn-warning btn-sm">검색</button>
					</form>
				</div>
			</div>
			...
			<c:if test="${rq.getLoginedMemberId() != 0 }">
				<div class="mt-2 flex justify-end">
					<a class="btn btn-warning btn-sm" href="write">글쓰기</a>
				</div>
			</c:if>
